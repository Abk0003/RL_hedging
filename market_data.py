...
SMA_10= SPY["Close"] / SPY["Close"].rolling(10).mean() - 1
SMA_50= SPY["Close"] / SPY["Close"].rolling(50).mean() - 1
...
# CONFIG
# ============================================================
ROLL_WINDOW = 504   # ~2 trading years, used for all rolling z-scores
MIN_PERIODS = 252   # require at least 1yr of history before z-score is valid
...
# Normalizes a series using only trailing window stats (no lookahead,
# and adapts to slow secular drift instead of using a single fixed
# train-period mu/std baked in forever).
# ============================================================
...
term_str = vix3m - vix  # already stationary (difference, bounded behavior)
...
# Price-level features converted to RATIOS (stationary) instead of raw levels.
# This fixes the SMA_10/SMA_50 issue where raw price-level features drift
# permanently out of distribution as SPY's nominal price rises over decades.
SMA_10 = (SPY["Close"] / SPY["Close"].rolling(10).mean() - 1).squeeze()
SMA_50 = (SPY["Close"] / SPY["Close"].rolling(50).mean() - 1).squeeze()
trend  = (SMA_10 / (SMA_50 + 1e-8) - 1)  # ratio of ratios, already relative

VIX_change = VIX["Close"].pct_change().squeeze()

# VIX_ma5 / VIX_ma20 are raw VIX levels -> same non-stationarity risk as SMA.
# We keep them ONLY as inputs to the (stationary) VIX_slope difference,
# and do NOT feed the raw levels into the model directly.
VIX_ma5  = VIX["Close"].rolling(5).mean().squeeze()
VIX_ma20 = VIX["Close"].rolling(20).mean().squeeze()
VIX_slope = VIX_ma5 - VIX_ma20  # difference of levels -> stationary-ish
...
# ============================================================
# APPLY ROLLING Z-SCORE NORMALIZATION TO EVERYTHING
# (instead of a single fixed train-period mu/std at the end)
# Series that are already differences/ratios still benefit from this,
# since their *scale* can drift even if their mean is roughly stationary
# (e.g. vol-of-vol regimes change over 20 years).
# ============================================================
...
zscore20_z = rolling_zscore(zscore_20)   # double z-score is fine: this re-normalizes its own scale drift
...
# NOTE: raw VIX_ma5 / VIX_ma20 levels deliberately excluded (see above);
# only the stationary VIX_slope (now z-scored) is included.
# ============================================================
feature_dict = {
    "rv21": rv21_z,
    "rv30": rv30_z,
    ...
    "trend": trend_z,
    "VIX_change": vixchg_z,
    "VIX_slope": vixslope_z,
    "zscore_20": zscore20_z,
    "realized_vol_change": rvolchg_z,
    "vix_vol": vixvol_z,
    "lag_1": lag1_z,
    "lag_2": lag2_z,
    "lag_5": lag5_z,
...
# No additional fixed-window mu/std normalization here -- rolling
# z-scores above already use only trailing/past data at each point,
# so there is no train/test leakage and no stale fixed baseline.
# ============================================================
train_end = "2021-12-31"
val_end = "2023-01-01"
